...
void Attack::Step()
{
	...
	if (m_isActive != 0)
	{
		...
		//m_pos.x += Attack_Len;
		m_timer += 1.0f;
		//g_Attack.m_timerがアクティブタイムより大きくなったら消す
		if (m_timer > Attack_ACTIVE_TIME)
		{
			m_timer = 0;
			m_isActive = 0;
...
void Attack::Draw()
{
	...
	if (m_cooltime > 0)
	{
		DrawFormatString(40, 75, GetColor(255, 0, 0), "Attack クールタイム : %.1f", m_cooltime);
	}
}
...
void Attack::Request(VECTOR pos,bool turn)
{
	// クールタイム中は発射不可
	//if (m_cooltime > 0.0f)
	//{
